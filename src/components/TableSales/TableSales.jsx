...
const TableSales = ({ data, columns, actions }) => {
    ...
    return (
        <div className="table-sales-cont">
            <ToastContainer
                position="top-right"
                autoClose={5000}
                ...
                <TableBody items={items}>
                    {(item) => (
                        < TableRow key={item.id}>
                            {(columnKey) => <TableCell className={(columnKey === "actions") ? "actions-cont" : ''}>{(columnKey === "actions") ? getActionButtons(item.id) : (columnKey === 'status') ? (item.status === 'Approved') ? <span className="status-span">{getKeyValue(item, columnKey)}</span> : <span>{getKeyValue(item, columnKey)}</span> : getKeyValue(item, columnKey)}</TableCell>}
                        </TableRow>
                    )}
                </TableBody>
            </Table>
